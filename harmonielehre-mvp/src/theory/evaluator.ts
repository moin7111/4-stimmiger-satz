...
import { checkParallels, checkRangesAndCrossing } from "./checker";
...
export function evaluateUserVoicing(
  ...
  if (!contains) {
    issues.push({ type: "not_in_allowed_voicings", message: "Voicing nicht in erlaubten Sätzen (Verdopplung/Umkehrung/Bereiche prüfen)." });
  }
