    ...
    <div className="max-w-5xl mx-auto p-6 space-y-8">
      ...
      <section>
        ...
        <div className="flex gap-3 flex-wrap">
          ...
          <Link href="/api/export?type=by-project" className="px-3 py-2 border rounded">Nach Projekten (CSV)</Link>
          <Link href="/api/export?type=by-class-zip" className="px-3 py-2 border rounded">Einzelne Klassen (ZIP)</Link>
        </div>
      </section>

      <section>
        <h2 className="text-xl font-semibold mb-2">Zuteilung</h2>
        <button onClick={() => fetch("/api/allocate", { method: "POST" })} className="px-4 py-2 bg-emerald-600 text-white rounded">Zuteilen (Ranglisten-Modell)</button>
      </section>
    </div>
  );
}
